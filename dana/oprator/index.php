...
if (isset($_POST['login'])) {
    ...
    // Cek user dengan role operator
    $query = "SELECT * FROM users WHERE username = ? AND thajaran = ? AND role = 'operator'";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $username, $thajaran);  // Bind parameter untuk username dan tahun ajaran
    $stmt->execute();
    ...
    if ($user && password_verify($password, $user['password'])) {
        ...
        $_SESSION['role']      = $user['role']; // Diperbaiki dari $user['operator']
